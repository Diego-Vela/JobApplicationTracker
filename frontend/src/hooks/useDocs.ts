...
export async function uploadToStorage(file: File): Promise<{ url: string }> {
  ...
  const { url, fields, file_url, max_size } = await apiSend<{
    url: string;
    fields: Record<string, string>;
    file_url: string;
    max_size: number;
  }>(`/files/presign?${q.toString()}`, "POST");
...
export function useDocs(kind: DocType) {
  ...
  const deletePath = (id: string) =>
    kind === "resume" ? `/files/resumes/${id}` : `/files/cv/${id}`;
  ...
  const remove = async (id: string) => {
    try {
      await apiSend<void>(deletePath(id), "DELETE");
