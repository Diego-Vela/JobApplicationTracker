// components/applications/StatusMNenu.tsx
import { useEffect, useState } from "react";
import { STATUS_LABELS, UI_TO_API, API_TO_UI } from "./statusMaps";
...
export default function StatusMenu({
  ...
  onChanged?: (ui: UIStatus) => void;
}) {
  const [open, setOpen] = useState(false);
  const uiValue: UIStatus = apiStatus ? API_TO_UI[apiStatus] : "applied";
  ...
  async function move(next: UIStatus) {
    ...
    <div className="relative" data-menu-for={appId}>
      <button
        onClick={() => setOpen((o) => !o)}
        className="rounded-full border px-2.5 py-1 text-xs capitalize text-gray-700 bg-white hover:bg-gray-50"
        aria-haspopup="listbox"
        aria-expanded={open}
      >
        {STATUS_LABELS[uiValue]}
      </button>
            ...
            <li key={opt}>
              <button
                className={[
                  ...
                  opt === uiValue ? "font-semibold" : "",
                ].join(" ")}
                onClick={() => move(opt)}
                role="option"
                aria-selected={opt === uiValue}
              >
                {STATUS_LABELS[opt]}
              </button>
            </li>
